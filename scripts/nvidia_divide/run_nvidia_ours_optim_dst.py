...
data_root_dir = '<data_dir of nvidia dataset>'
exp_root_dir = f'./exp/nvidia_divide_{version}'
profile_root_dir = f'./profile/nvidia_divide_{version}'

if exp_setting == 'native':
    prep_cfg_file = f"{profile_root_dir}/nvidia_fit_dst_{abla_setting}.yaml"
else:
    print('No such exp_setting.')
    exit(-1)

assert os.path.exists(prep_cfg_file)
...
for scene in scene_names:
    ...
    for dst_height in range(1, stack_depth_max+1):
        if exp_setting == 'native':
            dst_cfg_file = f"{scene_dir}/nvidia_fit_dst_{abla_setting}_h_{dst_height}.yaml"
        else:
            print('No such exp_setting.')
            exit(-1)
        if os.path.exists(dst_cfg_file): continue
