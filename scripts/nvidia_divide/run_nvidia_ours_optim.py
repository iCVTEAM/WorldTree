...
if exp_setting == 'native':
    prep_cfg_file = f"{profile_root_dir}/nvidia_fit_{abla_setting}.yaml"
else:
...
for scene in scene_names:
    ...
    if exp_setting == 'native':
        dst_cfg_file = f"{scene_dir}/nvidia_fit_dst_{abla_setting}_h_0.yaml"
    else:
        print('No such exp_setting.')
        exit(-1)
    dst_cfg_dist = optim_cfg_dict
