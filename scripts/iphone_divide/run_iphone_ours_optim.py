...
if exp_setting == 'native':
    prep_cfg_file = f"{profile_root_dir}/iphone_fit.yaml"
else:
...
for scene in scene_names:
    ...
    if not skip_training:
        ...
        pycode = f"CUDA_VISIBLE_DEVICES={GPU_ID} python mosca_reconstruct.py --ws {data_root_dir}/{scene} --cfg {prep_cfg_file}  --model_path {scene_dir} >> {scene_log_dir} 2>&1"
        rst = os.system(pycode)
        assert rst == 0
    if not skip_eval:
        scene_log_dir = os.path.join(scene_dir, "eval_log.txt")
        pycode = f"CUDA_VISIBLE_DEVICES={GPU_ID} python mosca_onlyeval.py --ws {data_root_dir}/{scene} --cfg {prep_cfg_file}  --model_path {scene_dir} >> {scene_log_dir} 2>&1"
        rst = os.system(pycode)
        assert rst == 0
