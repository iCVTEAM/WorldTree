...
def load_imgs_from_dir(src):
    ...
    # import pdb; pdb.set_trace() # xx = [os.path.join(img_dir, it) for it in img_fns]
    # img_list = [imageio.imread(os.path.join(img_dir, it))[..., :3] for it in img_fns]
    img_list = []
    for it in img_fns:
        # if '0_00270.png' in it: import pdb; pdb.set_trace()
        # print('test:', os.path.join(img_dir, it))
        img = imageio.imread(os.path.join(img_dir, it))[..., :3]
        img_list.append(img)
...
def preprocess(
    ...
    use_gt_mask = getattr(pre_cfg, "use_gt_mask", False)
    ...
    if not use_gt_mask:
        ...
        s2d.load_gt_mask()
        sample_mask = s2d.gt_mask > 0.5
        
    imageio.mimsave(
        # osp.join(ws, "epi_resample_mask.gif"),
        os.path.join(ws, "epi_resample_mask.gif"), 
