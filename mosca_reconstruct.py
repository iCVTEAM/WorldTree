...
def photometric_warmup(ws, log_path, fit_cfg):
    ...
    if not use_gt_mask:
        ...
        s2d.register_2d_identification(
            static_2d_mask=s2d.gt_mask < 0.5, dynamic_2d_mask=s2d.gt_mask > 0.5
        )
...
def scaffold_reconstruct(ws, log_path, fit_cfg):
    ...
    if not use_gt_mask:
        ...
        from lib_moca.epi_helpers import identify_tracks_gt_mask
        track_static_selection, track_dynamic_selection = identify_tracks_gt_mask(track_mask, static_cnt=1, dynamic_cnt=DYN_ID_CNT)
        ...
        # pass # move to static_reconstruct
...
def photometric_reconstruct(ws, log_path, fit_cfg):
    ...
    if getattr(fit_cfg, "photo_d_model_fetch_only_error_th", -1) > 0 and getattr(fit_cfg, "photo_static_warm_steps", -1) > 0:
        photo_data = np.load(osp.join(log_path, "photo_warmup_rendered.npz"))
    ...
    mid_split = getattr(fit_cfg, "mid_split", True)
    custom_split = getattr(fit_cfg, "custom_split", "gradsplit")
    if not mid_split and custom_split == "flowsplit":
        flow_presplit_json_dir = os.path.join(ws, "flow_presplit.json")
        import json
        with open(flow_presplit_json_dir, 'r') as flow_presplit_json_file:
            flow_presplit_json_data = json.load(flow_presplit_json_file)
            flow_presplit_json_file.close()
        assert "flow_split_infos" in flow_presplit_json_data
        flow_split_infos = flow_presplit_json_data["flow_split_infos"]
        fit_cfg["flow_split_infos"] = flow_split_infos

    if getattr(fit_cfg, "stack_depth_max", 3) > 0 and getattr(fit_cfg, "use_fachain", True):
        from lib_mosca.photo_recon_utils import offload_model
        s2d.to(torch.device("cpu"))
        offload_model(s2d)
        cams.to(torch.device("cpu"))
        offload_model(cams)
        d_model.scf.to(torch.device("cpu"))
        offload_model(d_model.scf)
        d_model.to(torch.device("cpu"))
        offload_model(d_model)
        print("VRAM Detection photometric fit start.", torch.cuda.memory_allocated()/(1024**3))
    
    activate_monitor_vram_time = getattr(fit_cfg, "activate_monitor_vram_time", False)
    if activate_monitor_vram_time:
        real_gpu_ids = getattr(fit_cfg, "gpu_id", "0")
        real_gpu_ids_list = real_gpu_ids.split(',')
        gpu_ids_list = [gpu_idx for gpu_idx in range(len(real_gpu_ids_list))]
        vram_monitor = MultiGPUMonitorPYNVML(gpu_ids=gpu_ids_list, sampling_interval=0.02)
        # NOTE: start monitor vram
        vram_monitor.start_monitoring()
        # NOTE: start record time
        import time
        photometric_fit_bfs_start_time = time.time()
    tree_root, bfs_list = photo_solver.photometric_fit_bfs_fachain(
        s2d=s2d,
        ...
        ), # NOTE: Maybe OOM Error, since in the profile yaml file may not have this item (defalt 10,000). If OOM, set this to 32 in the corresponding profile yaml file (in the demo, tum profiles, this value is 32).
        # Dyn clean
        ...
        stack_depth_max=getattr(fit_cfg, "stack_depth_max", 3), 
        dyn_gs_num_max=getattr(fit_cfg, "gs_dynamic_n_init", 30000), 
        additional_mask=d_model_add_mask, 
        mid_split=getattr(fit_cfg, "mid_split", False), 
        min_interval=getattr(fit_cfg, "min_interval", 12), 
        max_depth_for_static=getattr(fit_cfg, "max_depth_for_static", 2), 
        max_depth_for_cams=getattr(fit_cfg, "max_depth_for_cams", 100), 
        total_steps_list = getattr(fit_cfg, "total_steps_list", []),
        decay_start_list = getattr(fit_cfg, "decay_start_list", []),
        optim_cam_after_steps_list = getattr(fit_cfg, "optim_cam_after_steps_list", []),
        use_fachain = getattr(fit_cfg, "use_fachain", True),
        fit_cfg = fit_cfg, 
    )
    if activate_monitor_vram_time:
        # NOTE: stop record time
        photometric_fit_bfs_end_time = time.time()
        photometric_fit_bfs_total_time = photometric_fit_bfs_end_time - photometric_fit_bfs_start_time
        record_time_dir = osp.join(log_path, f"record_time_photometric_fit_bfs.txt")
        with open(record_time_dir, 'a+') as record_time_file:
            record_time_content = f"RECORD TIME (photometric_fit_bfs): {photometric_fit_bfs_total_time} seconds."
            print(record_time_content)
            record_time_file.write(record_time_content+"\n")
            record_time_file.close()
        # NOTE: stop monitor vram
        vram_monitoring_results = vram_monitor.stop_monitoring()
        vram_monitor.print_results(vram_monitoring_results)
        vram_content = vram_monitor.export_results(vram_monitoring_results)
        record_vram_dir = osp.join(log_path, f"record_vram_photometric_fit_bfs.txt")
        with open(record_vram_dir, 'a+') as record_vram_file:
            record_vram_file.write(vram_content)
            record_vram_file.close()
...
if __name__ == "__main__":
    ...
    if not getattr(cfg, "resume_photometric_reconstruct", False):
    ...
    activate_monitor_vram_time = getattr(cfg, "activate_monitor_vram_time", False)
    if activate_monitor_vram_time:
        real_gpu_ids = getattr(cfg, "gpu_id", "0")
        real_gpu_ids_list = real_gpu_ids.split(',')
        gpu_ids_list = [gpu_idx for gpu_idx in range(len(real_gpu_ids_list))]
        all_vram_monitor = MultiGPUMonitorPYNVML(gpu_ids=gpu_ids_list, sampling_interval=0.02)
        # NOTE: start monitor vram of all process
        all_vram_monitor.start_monitoring()
    ...
    tree_root, bfs_list = photometric_reconstruct(args.ws, logdir, cfg)

    if activate_monitor_vram_time:
        # NOTE: stop monitor vram of all process
        all_vram_monitoring_results = all_vram_monitor.stop_monitoring()
        all_vram_monitor.print_results(all_vram_monitoring_results)
        all_vram_content = all_vram_monitor.export_results(all_vram_monitoring_results)
        record_vram_dir = osp.join(logdir, f"record_vram_photometric_fit_all.txt")
        with open(record_vram_dir, 'a+') as record_vram_file:
            record_vram_file.write(all_vram_content)
            record_vram_file.close()
