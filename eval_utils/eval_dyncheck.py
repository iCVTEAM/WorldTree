...
def eval_dycheck_ours(
    ...
    gt_rgb_fns_all = sorted(os.listdir(gt_rgb_dir))
    gt_mask_fns_all = sorted(os.listdir(gt_mask_dir))
    pred_fns = sorted(os.listdir(pred_dir))

    gt_rgb_fns = []
    gt_mask_fns = []
    for each_pred_fn in pred_fns:
        if each_pred_fn in gt_rgb_fns_all:
            gt_rgb_fns.append(each_pred_fn)
        if each_pred_fn in gt_mask_fns_all:
            gt_mask_fns.append(each_pred_fn)
