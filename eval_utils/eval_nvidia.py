...
def eval_nvidia_ours_dir(gt_dir, pred_dir, report_dir, fixed_view_id=0, N_images=-1, left_bound=-1, right_bound=-1):
    assert N_images != -1
    lpips_loss = lpips.LPIPS(net="alex")  # best forward scores
    assert osp.exists(gt_dir)
    assert osp.exists(pred_dir)
    ...
    if left_bound == -1 and right_bound == -1:
        left_bound = 0
        right_bound = N_images - 1

    gt_images_list = sorted(os.listdir(gt_dir))
    gt_images_list = gt_images_list[left_bound:right_bound+1]

    psnr_list, ssim_list, lpips_list = [], [], []
    results = []
    # for time in range(time_start, time_end):  # Fix view fixed_view_id, change time
    for image_name in gt_images_list:
        time = int(image_name[6:9])
        gt_fn = osp.join(gt_dir, f"v{fixed_view_id:03d}_t{time:03d}.png")
        pred_fn = osp.join(pred_dir, f"v{fixed_view_id:03d}_t{time:03d}.png")
        img_true = cv2.imread(gt_fn)
        img = cv2.imread(pred_fn)
        ...
        results.append({"image_name": f"v{fixed_view_id:03d}_t{time:03d}.png", "time": time, "psnr": _psnr, "ssim": _ssim, "lpips": _lpips})
    ...
    # build pandas dataframe and save to report dir an excel
    results = [
        {"image_name": "all", "time": "ave", "psnr": ave_psnr, "ssim": ave_ssim, "lpips": ave_lpips}
    ] + results
    # save each row as a time frame
    df = pd.DataFrame(results)
    save_report_fn = osp.join(report_dir, "nvidia_render_metrics.xlsx")
...
def eval_nvidia_mask_ours_dir(gt_dir, mask_dir, pred_dir, report_dir, fixed_view_id=0, N_images=-1, left_bound=-1, right_bound=-1):
    assert N_images != -1
    lpips_loss = lpips.LPIPS(net="alex")  # best forward scores, following som, mosca
    assert osp.exists(gt_dir)
    assert osp.exists(mask_dir)
    assert osp.exists(pred_dir)
    if os.path.exists(report_dir):
        os.system(f'rm -r {report_dir}')
    os.makedirs(report_dir, exist_ok=True)
    ...
    for image_name in gt_images_list:
        time = int(image_name[6:9])
        gt_fn = osp.join(gt_dir, f"v{fixed_view_id:03d}_t{time:03d}.png")
        mask_fn = osp.join(mask_dir, f"v{fixed_view_id:03d}_t{time:03d}.png")
        pred_fn = osp.join(pred_dir, f"v{fixed_view_id:03d}_t{time:03d}.png")
        img_true = cv2.imread(gt_fn)
        mask_true = cv2.imread(mask_fn)
        mask_true = (mask_true > 0)
        img = cv2.imread(pred_fn)
        white_background = np.ones_like(img)
        img_true[mask_true == False] = 255
        img[mask_true == False] = 255

        # _psnr = cv2.PSNR(img_true, img)
        # _ssim = structural_similarity(img_true, img, multichannel=True)
        # import pdb; pdb.set_trace()
        _psnr = cv2.PSNR(img_true[mask_true], img[mask_true])
        _ssim = structural_similarity(img_true[mask_true], img[mask_true])
        _lpips = lpips_loss.forward(im2tensor(img_true), im2tensor(img)).item()
        ...
        results.append({"image_name": f"v{fixed_view_id:03d}_t{time:03d}.png", "time": time, "mpsnr": _psnr, "mssim": _ssim, "mlpips": _lpips})
        ...
        errmap = cv2.putText(
            errmap,
            f"NORM[{factor:.3f}] MPSNR={_psnr:.3f}, MSSIM={_ssim:.3f}, MLPIPS={_lpips:.3f}",
            (10, 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (255, 255, 255),
        ...
        save_viz = np.concatenate([img_true, img, errmap], axis=1)
        save_name = osp.join(
            viz_dir, f"v{fixed_view_id:03d}_t{time:03d}_mpsnr={_psnr:.3f}.png"
        )
        cv2.imwrite(save_name, save_viz)
    ...
    ave_lpips = np.mean(lpips_list)
    # save the txt of three metrics to report dir
    with open(osp.join(report_dir, "nvidia_render_metrics_masked.txt"), "w") as f:
        f.write(f"Mask-PSNR: {ave_psnr:.6f}\n")
        f.write(f"Mask-SSIM: {ave_ssim:.6f}\n")
        f.write(f"Mask-LPIPS: {ave_lpips:.6f}\n")

    # build pandas dataframe and save to report dir an excel
    results = [
        {"image_name": "all", "time": "ave", "mpsnr": ave_psnr, "mssim": ave_ssim, "mlpips": ave_lpips}
    ] + results
    # save each row as a time frame
    df = pd.DataFrame(results)
    save_report_fn = osp.join(report_dir, "nvidia_render_metrics.xlsx")
    ...
    logging.info(f"Saved the evaluation report to {report_dir}")
    logging.info(
        f"Metric: Mask-PSNR={ave_psnr:.6f}, Mask-SSIM={ave_ssim:.6f}, Mask-LPIPS={ave_lpips:.6f}"
    )
    return ave_psnr, ave_ssim, ave_lpips
