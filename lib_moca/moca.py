...
from epi_helpers import analyze_track_epi, identify_tracks, identify_tracks_gt_mask
...
def rescale_camera_pose(
    ...
    # avoid singular case # NOTE: adding check nan
    if s_optimal < 0.00001 or (-(a * b).sum() / (b * b).sum()).isnan().any():
        logging.warning(
...
def moca_solve_gt_mask(
    ...
    #     epierr_list, epi_th
    # )
    track_static_selection, track_dynamic_selection = identify_tracks_gt_mask(track_mask, static_cnt=12, dynamic_cnt=12) # NOTE: HARD-CODE
