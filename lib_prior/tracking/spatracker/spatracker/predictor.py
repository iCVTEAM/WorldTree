...
class SpaTrackerPredictor(torch.nn.Module):
    ...
    def forward(
        ...
        return tracks, visibilities, T_Firsts, None
    ...
    def _compute_sparse_tracks(
        ...
        tracks, __, visibilities = self.model(
            rgbds=rgbds, queries=queries, iters=6, wind_S=wind_length, intrs=intrs
        )
        ...
        for i in range(len(queries)):
            queries_t = queries[i, : tracks.size(2), 0].to(torch.int64)  # T
    ...
    def _compute_backward_tracks(
        self, video, queries, tracks, visibilities, intrs=None
    ):
        ...
        inv_tracks, __, inv_visibilities = self.model(
            rgbds=inv_video, queries=inv_queries, iters=6, intrs=intrs
        )
        ...
        mask = (arange < queries[:, None, :, 0]).unsqueeze(-1).repeat(1, 1, 1, 3)
