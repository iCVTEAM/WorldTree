...
def bilinear_sample2d(im, x, y, return_inbounds=False):
    ...
    if len(im.shape) == 5:
        im_flat = (im.permute(0, 3, 4, 1, 2)).reshape(B * H * W, N, C)
        # check the index in in she safe range
        assert idx_y0_x0.max() < len(
            im_flat
        ), f"idx_y0_x0.max()={idx_y0_x0.long().max()}, len(im_flat)={len(im_flat)}"
        assert idx_y0_x1.max() < len(
            im_flat
        ), f"idx_y0_x1.max()={idx_y0_x1.long().max()}, len(im_flat)={len(im_flat)}"
        assert idx_y1_x0.max() < len(
            im_flat
        ), f"idx_y1_x0.max()={idx_y1_x0.long().max()}, len(im_flat)={len(im_flat)}"
        assert idx_y1_x1.max() < len(
            im_flat
        ), f"idx_y1_x1.max()={idx_y1_x1.long().max()}, len(im_flat)={len(im_flat)}"
        i_y0_x0 = torch.diagonal(im_flat[idx_y0_x0.long()], dim1=1, dim2=2).permute(
            0, 2, 1
        )
        i_y0_x1 = torch.diagonal(im_flat[idx_y0_x1.long()], dim1=1, dim2=2).permute(
