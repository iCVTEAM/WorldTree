...
class TAPIR(nn.Module):
  ...
  def __init__(
    ...
    self.resnet_torch = pt_nets.ResNet(
    ...
    self.torch_pips_mixer = pt_nets.PIPSMLPMixer(
        input_dim, dim, use_causal_conv=self.use_casual_conv
    )

    if extra_convs:
      self.extra_convs = pt_nets.ExtraConvs()
    else:
  ...
  def get_query_features(
    ...
    for i, resolution in enumerate(resize_im_shape):
      if pt_utils.is_same_res(curr_resolution, resolution):
        query_feats.append(query_feats[-1])
        hires_query_feats.append(hires_query_feats[-1])
        continue
      position_in_grid = pt_utils.convert_grid_coordinates(
          query_points,
          shape[1:4],
          feature_grid[i].shape[1:4],
          coordinate_format='tyx',
      )
      position_in_grid_hires = pt_utils.convert_grid_coordinates(
          query_points,
          shape[1:4],
      ...
      interp_features = pt_utils.map_coordinates_3d(
          feature_grid[i], position_in_grid
      )
      hires_interp = pt_utils.map_coordinates_3d(
  ...
  def get_feature_grids(
    ...
    if refinement_resolutions is None:
      refinement_resolutions = pt_utils.generate_default_resolutions(
    ...
    for resolution in all_required_resolutions:
      ...
      if not pt_utils.is_same_res(curr_resolution, resolution):
        if pt_utils.is_same_res(curr_resolution, video.shape[-3:-1]):
          video_resize = video
        else:
          video_resize = pt_utils.bilinear(video, resolution)
  ...
  def estimate_trajectories(
    ...
    def train2orig(x):
      return pt_utils.convert_grid_coordinates(
          x,
    ...
    for ch in range(0, num_queries, query_chunk_size):
      ...
      if query_points_in_video is not None:
        ...
        infer_query_points = pt_utils.convert_grid_coordinates(
            infer_query_points,
  ...
  def refine_pips(
    ...
    for pyridx, (query, grid) in enumerate(zip(target_feature, pyramid)):
      ...
      coords = pt_utils.convert_grid_coordinates(
      ...
      neighborhood = pt_utils.map_coordinates_2d(grid, coords2)
    ...
    x = pt_utils.einshape('bnfc->(bn)fc', mlp_input)

    if causal_context is not None:
      for k, v in causal_context.items():
        causal_context[k] = pt_utils.einshape('bn...->(bn)...', v)
    res, new_causal_context = self.torch_pips_mixer(
        x.float(), causal_context, get_causal_context
    )
    res = pt_utils.einshape('(bn)fc->bnfc', res, b=mlp_input.shape[0])
    if get_causal_context:
      for k, v in new_causal_context.items():
        new_causal_context[k] = pt_utils.einshape(
            '(bn)...->bn...', v, b=mlp_input.shape[0]
        )

    pos_update = pt_utils.convert_grid_coordinates(
        res[..., :2],
        (resized_w, resized_h),
        (orig_w, orig_h),
    )
  ...
  def tracks_from_cost_volume(
    ...
    cost_volume = pt_utils.einshape('tbnhw->(tbn)hw1', cost_volume)
    ...
    pos_rshp = pt_utils.einshape('(tb)hw1->t(b)hw1', pos, t=shape[0])

    pos = pt_utils.einshape(
    ...
    points = pt_utils.heatmaps_to_points(pos, im_shp, query_points=query_points)
    ...
    expected_dist = pt_utils.einshape(
        '(tbn)1->bnt', occlusion[..., 1:2], n=shape[2], t=shape[0]
    )
    occlusion = pt_utils.einshape(
