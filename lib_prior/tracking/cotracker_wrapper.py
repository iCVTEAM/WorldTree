...
def get_cotracker(device, online_flag=True, cotrakcer_version="3"):
    if online_flag:
        ...
        # ).to(device)
        cotracker = torch.hub.load(
            "./weights/co-tracker", f"cotracker{cotrakcer_version}_online", source='local', verbose=True, pretrain=True
        ).to(device)
    else:
        postfix = "_offline" if cotrakcer_version == "3" else ""
        # cotracker = torch.hub.load(
        #     "./weights/co-tracker", f"cotracker{cotrakcer_version}{postfix}"
        # ).to(device)
        cotracker = torch.hub.load(
            "./weights/co-tracker", f"cotracker{cotrakcer_version}{postfix}", source='local', verbose=True, pretrain=True
        ).to(device)
    cotracker.eval()
    return cotracker
