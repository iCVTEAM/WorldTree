...
class Saved2D(nn.Module):
    ...
    @torch.no_grad
    def load_gt_mask(self, mask_dirname="train_masks"):
        img_dir = osp.join(self.ws, mask_dirname)
        if osp.exists(img_dir):
            ...
            img_names = [osp.splitext(f)[0] for f in img_fns]
            images = [imageio.imread(osp.join(img_dir, img_fn)) for img_fn in img_fns]
            images = torch.Tensor(np.stack(images)) / 255.0  # T,H,W
            images = (~(images > 0.)).float()
        else:
            raise ValueError(f"Cannot find masks in {img_dir}")
        # assign
        assert self.frame_names == img_names
        self.register_gradfree_buffer("gt_mask", images.detach())
        return self
