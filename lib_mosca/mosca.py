...
class MoSca(nn.Module):
    def __init__(
        ...
        self.register_buffer("skinning_k", torch.tensor(skinning_k).long().to(init_device))
        self.register_buffer("topo_dist_top_k", torch.tensor(topo_dist_top_k).long().to(init_device))
        self.register_buffer("topo_sample_T", torch.tensor(topo_sample_T).long().to(init_device))
        self.register_buffer("topo_th_ratio", torch.tensor(topo_th_ratio).to(init_device))
        ...
        if spatial_unit_hard_set is None or spatial_unit_hard_set <= 0.0:
            ...
            if init_device != torch.device("cpu"):
                torch.cuda.synchronize(init_device)
            try:
                spatial_unit = spatial_unit_factor * __identify_spatial_unit_from_curves__(
                    self._node_xyz, self._node_certain, K=self.skinning_k
                )
            except:
    ...
    @torch.no_grad()
    def resample_time(self, new_tids, new_node_certain, mode="linear"): # NOTE: in fact, self._t_list is useless, only for resampling, so we did not change this func, and constuct the params input for using it.
