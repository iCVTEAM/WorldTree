...
class DynSCFGaussian(nn.Module):
    ...
    #################################################################
    # * buffers
    # max_scale # NOTE: shape: 1
    # min_sacle # NOTE: shape: 1
    # max_sph_order # NOTE: shape: 1

    # attach_ind # NOTE: shape: N
    # ref_time # NOTE: shape: N

    # xyz_gradient_accum # NOTE: shape: N
    # xyz_gradient_denom # NOTE: shape: N
    # corr_gradient_accum # NOTE: shape: N
    # corr_gradient_denom # NOTE: shape: N
    # max_radii2D # NOTE: shape: N

    # * parameters
    # _xyz # NOTE: shape: Nx3
    # _rotation # NOTE: shape: Nx4
    # _scaling # NOTE: shape: Nx3
    # _opacity # NOTE: shape: Nx1
    # _features_dc # NOTE: shape: Nx3
    # _features_rest # NOTE: shape: Nx45
    # _skinning_weight # NOTE: shape: Nx16
    # _dynamic_logit # NOTE: shape: Nx1
    #################################################################
    ...
    def get_xyz_specified_mosca(self, scaffold, attach_ind_to_scf):
        if self.leaf_local_flag:
            nn_ref_node_xyz, sk_ref_node_quat = scaffold.get_async_knns(
                self.ref_time, attach_ind_to_scf[:, None]
            )
            nn_ref_node_xyz = nn_ref_node_xyz.squeeze(1)
            nn_ref_node_R_wi = q2R(sk_ref_node_quat.squeeze(1))
            return (
    ...
    def get_R_mtx_specified_mosca(self, scaffold, attach_ind_to_scf):
        if self.leaf_local_flag:
            _, sk_ref_node_quat = scaffold.get_async_knns(
                self.ref_time, attach_ind_to_scf[:, None]
            )
            nn_ref_node_R_wi = q2R(sk_ref_node_quat.squeeze(1))
            return torch.einsum("nij,njk->nik", nn_ref_node_R_wi, q2R(self._rotation))
    ...
    def unset_surface_deform(self):
        # * different to using RBF field approximate the deformation field (more flexible when changing the scf topology), set the deformation to surface model, the skinning is saved on each GS
        self.w_correction_flag = torch.tensor(False).to(self.device)
        self.scf.fixed_topology_flag = torch.tensor(False).to(self.device)
        return
    ...
    def forward(self, t: int, active_sph_order=None, nn_fusion=None, ref_time_mask=None):
        ...
        if ref_time_mask is not None:
            ref_time_mask = ref_time_mask.to(mu_live.device)
            mu_live = mu_live[ref_time_mask] if mu_live is not None else None
            fr_live = fr_live[ref_time_mask] if fr_live is not None else None
            s = s[ref_time_mask] if s is not None else None
            o = o[ref_time_mask] if o is not None else None
            sph = sph[ref_time_mask] if sph is not None else None

        return mu_live, fr_live, s, o, sph
    
    # @torch.no_grad()
    def forward_specified_mosca(self, t: int, query_t, scaffold: MoSca, attach_ind_to_scf, active_sph_order=None, ref_time_mask=None):
        ...
        if self.fast_inference_flag: # TODO: not implemented
            print("fast_inference_flag for forward_specified_mosca: not implemented.")
            mu_live, fr_live = scaffold.fast_warp(
                target_tid=t,
                # all below are baked
                sk_ind=self.baked_sk_ind,
                sk_w=self.baked_sk_w,
            ...
            mu_live, fr_live = scaffold.warp(
                attach_node_ind=attach_ind_to_scf,
                query_xyz=self.get_xyz_specified_mosca(self.scf, self.attach_ind),
                query_dir=self.get_R_mtx_specified_mosca(self.scf, self.attach_ind),
                query_tid=query_t,
                target_tid=t,
                skinning_w_corr=(
                    self._skinning_weight if self.w_correction_flag else None
    ...
    def record_xyz_grad_radii_fachain(self, viewspace_point_tensor_grad, radii, update_filter, ref_time_mask):
        # Record the gradient norm, invariant across different poses
        self.xyz_gradient_accum[ref_time_mask][update_filter] += torch.norm(
            viewspace_point_tensor_grad[update_filter, :2], dim=-1, keepdim=False
        )
        self.xyz_gradient_denom[ref_time_mask][update_filter] += 1
        self.max_radii2D[ref_time_mask][update_filter] = torch.max(
            self.max_radii2D[ref_time_mask][update_filter], radii[update_filter]
        )
        return
    ...
    def record_corr_grad_fachain(self, viewspace_point_tensor_grad, update_filter, ref_time_mask):
        # Record the gradient norm, invariant across different poses
        self.corr_gradient_accum[ref_time_mask][update_filter] += torch.norm(
            viewspace_point_tensor_grad[update_filter, :2], dim=-1, keepdim=False
        )
        self.corr_gradient_denom[ref_time_mask][update_filter] += 1
        return
    ...
    def resample_time(
        ...
        masked_ref_time = torch.zeros_like(self.ref_time).bool()
        for idx in range(len(self.ref_time)):
            tid = self.ref_time[idx]
            if tid in resampled_t_list: masked_ref_time[idx] = True
        self.attach_ind = self.attach_ind[masked_ref_time]
        self.ref_time = self.ref_time[masked_ref_time] - time_bias
        self.xyz_gradient_accum = self.xyz_gradient_accum[masked_ref_time]
        self.xyz_gradient_denom = self.xyz_gradient_denom[masked_ref_time]
        self.corr_gradient_accum = self.corr_gradient_accum[masked_ref_time]
        self.corr_gradient_denom = self.corr_gradient_denom[masked_ref_time]
        self.max_radii2D = self.max_radii2D[masked_ref_time]
        self._xyz = nn.Parameter(self._xyz[masked_ref_time])
        self._rotation = nn.Parameter(self._rotation[masked_ref_time])
        self._scaling = nn.Parameter(self._scaling[masked_ref_time])
        self._opacity = nn.Parameter(self._opacity[masked_ref_time])
        self._features_dc = nn.Parameter(self._features_dc[masked_ref_time])
        self._features_rest = nn.Parameter(self._features_rest[masked_ref_time])
        self._skinning_weight = nn.Parameter(self._skinning_weight[masked_ref_time])
        self._dynamic_logit = nn.Parameter(self._dynamic_logit[masked_ref_time])

    @torch.no_grad()
    def resample_time_lim(
        self, 
        resampled_t_list, 
        time_bias=0, 
        n_init=-1, 
        ...
        for idx in range(len(self.ref_time)):
            ...
            if tid in resampled_t_list: masked_ref_time[idx] = True
        if n_init > 0:
            interval_indices = torch.where(masked_ref_time)[0]
            interval_indices_num = len(interval_indices)
            if n_init < interval_indices_num:
                downsampled_indices = interval_indices[
                    torch.randperm(
                        interval_indices_num
                    )[:n_init]
                ]
                downsampled_masked_ref_time = torch.zeros_like(masked_ref_time).bool()
                downsampled_masked_ref_time[downsampled_indices] = True
                masked_ref_time = downsampled_masked_ref_time
                assert downsampled_masked_ref_time.sum() == n_init
