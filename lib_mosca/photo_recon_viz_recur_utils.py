...
def viz2d_total_video_interval(
    s2d: Saved2D,
    viz_vid_fn,
    start_from,
    end_at,
...
def viz2d_one_frame(
    ...
    if rgb_mask is None:
        rgb_mask = s2d.get_mask_by_key(loss_mask_type)[left_bound+model_tid].clone()
    _, rgb_loss_i, pred_rgb, gt_rgb = compute_rgb_loss(
        s2d.rgb[left_bound+model_tid].clone(), render_dict, rgb_mask
    )
    ...
    if append_depth:
        if dep_mask is None:
            dep_mask = rgb_mask * s2d.dep_mask[left_bound+model_tid]
        _, dep_loss_i, pred_dep, prior_dep = compute_dep_loss(
            s2d.dep[left_bound+model_tid].clone(), render_dict, dep_mask
        )
        ...
        try:
            _, normal_loss_i, pred_normal, gt_normal = compute_normal_loss(
                s2d.nrm[left_bound+model_tid].clone(), render_dict, dep_mask
            )
    ...
    if d_model is not None and append_dyn:
        ...
        _, dyn_rgb_loss_i, dyn_pred_rgb, dyn_gt_rgb = compute_rgb_loss(
            s2d.rgb[left_bound+model_tid].clone(), dyn_render_dict, rgb_mask
        )
...
def viz2d_total_video_interval_fachain(
    s2d: Saved2D,
    viz_vid_fn,
    start_from,
    end_at,
    ...
    for _ind, view_ind in tqdm(enumerate(range(start_from, min(end_at + 1, cams.T)))):
        ...
        # fix cam, vary time
        _frame = viz2d_one_frame_fachain(
            view_ind,
            s2d,
            cams,
            s_model,
...
def viz2d_one_frame_fachain(
    ...
    if d_model is not None:
        if len(fachain_d_model_list) == 0:
            ...
            # fachain_fused_d_model_dst = list(d_model(model_tid))
            gs5.append(d_model(model_tid))
            for fa_d_model_infodict_idx in range(0, len(fachain_d_model_list)):
                ...
                if (ref_time_mask is None) or (ref_time_mask is not None and ref_time_mask.sum() > 0):
                    gs5.append(fa_d_model_infodict["d_model"](model_tid + fa_d_model_infodict["leaf_left_bound"] - fa_d_model_infodict["left_bound"], ref_time_mask=ref_time_mask))
            #     fa_d_model_dst = d_model.forward_specified_mosca(
            #         t = model_tid + fa_d_model_infodict["leaf_left_bound"] - fa_d_model_infodict["left_bound"],
            #         query_t = d_model.ref_time + fa_d_model_infodict["leaf_left_bound"] - fa_d_model_infodict["left_bound"], 
            #         scaffold = fa_d_model_infodict["d_model"].scf, 
            #         attach_ind_to_scf = d_model.attach_ind, 
            #         ref_time_mask = ref_time_mask, 
            #     )
            #     fachain_fused_d_model_dst[0] += fa_d_model_dst[0] # mu
            #     fachain_fused_d_model_dst[1] += fa_d_model_dst[1] # rot
            # fachain_fused_d_model_dst[0] /= len(fachain_d_model_list) + 1
            # fachain_fused_d_model_dst[1] /= len(fachain_d_model_list) + 1
            # gs5.append(fachain_fused_d_model_dst)
    render_dict = render(gs5, cams.default_H, cams.default_W, cams.default_K, T_cw)
    if rgb_mask is None:
        rgb_mask = s2d.get_mask_by_key(loss_mask_type)[left_bound+model_tid].clone()
    _, rgb_loss_i, pred_rgb, gt_rgb = compute_rgb_loss(
        ...
        if len(fachain_d_model_list) == 0:
            ...
            for fa_d_model_infodict_idx in range(0, len(fachain_d_model_list)):
                ...
                if fa_d_model_infodict["do_fachain_ref_time_mask"]:
                    ...
                    ref_time_mask = torch.logical_and(ref_time_extend_mask, ref_time_mask)
                if (ref_time_mask is None) or (ref_time_mask is not None and ref_time_mask.sum() > 0):
                    gs_list.append(fa_d_model_infodict["d_model"](model_tid + fa_d_model_infodict["leaf_left_bound"] - fa_d_model_infodict["left_bound"], ref_time_mask=ref_time_mask))
            #     fa_d_model_dst = d_model.forward_specified_mosca(
            #         t = model_tid + fa_d_model_infodict["leaf_left_bound"] - fa_d_model_infodict["left_bound"],
            #         query_t = d_model.ref_time + fa_d_model_infodict["leaf_left_bound"] - fa_d_model_infodict["left_bound"], 
            #         scaffold = fa_d_model_infodict["d_model"].scf, 
            ...
            # fachain_fused_d_model_dst[1] /= len(fachain_d_model_list) + 1
            dyn_render_dict = render(
                gs_list, # [fachain_fused_d_model_dst],
                cams.default_H,
                cams.default_W,
                cams.default_K,
                T_cw,
