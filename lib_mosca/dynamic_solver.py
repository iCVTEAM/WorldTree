...
def geometry_scf_init(
    ...
    for step in tqdm(range(total_steps)):
        ...
        if step in reline_steps: # [] NOTE: for nvidia, iphone
        ...
        if step in update_all_topo_steps: # NOTE: [] for nvidia, iphone
            logging.info(
                f"As specified, update full topo at step={step} without any mask"
            )
            scf.update_topology(curve_mask=None, verbose=True)
        elif step % mlevel_resample_steps == 0 and step > 0: # NOTE: mlevel_resample_steps = 100 for nvidia, iphone
            if update_full_topo: # ! must be true for 2D tracks
                scf.update_topology(
